...
export async function remediateThreat(action: string, resourceId?: string) {
    ...
    if (!action.includes("StopLogging") && !action.includes("CloudTrail")) {
        ...
        // Ideally we check specific strings, but let's be lenient for the demo
        // If action is completely unrelated, maybe return error.
        // But for this task, the action summary is "CloudTrail Logging Disabled".
    }
